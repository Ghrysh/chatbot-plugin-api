...
def fix_file(filepath):
    ...
    # Find the form with action="/knowledge/generate"
    # We want to replace everything from @csrf to the end of the form.
    form_start = content.find('action="/knowledge/generate"')
    if form_start == -1:
        print(f"Could not find form in {filepath}")
        return
        
    csrf_idx = content.find('@csrf', form_start)
    if csrf_idx == -1:
        print(f"Could not find @csrf after form in {filepath}")
        return
        
    form_end = content.find('</form>', csrf_idx)
    if form_end == -1:
        print(f"Could not find </form> after @csrf in {filepath}")
        return

    # Check if we need embed specific hidden fields
    hidden_fields = ''
...
"""
    ...
    replacement = """@csrf""" + hidden_fields + """
                        <!-- VIEW 1: Form Input -->
...
"""
    new_content = content[:csrf_idx] + replacement + content[form_end:]
    with open(filepath, 'w') as f:
        f.write(new_content)
    print(f"Successfully replaced {filepath}")
